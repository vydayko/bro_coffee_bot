...
stepHandler.action(coffeesKeys, (ctx) => {
  ctx.wizard.state.coffeeSetup.coffeeType = ctx.match;
  
  ctx.reply('Step 2. Chose milk', Markup.inlineKeyboard([
...
const superWizard = new WizardScene('super-wizard',
  ...
  (ctx) => {
    ctx.wizard.state.coffeeSetup.syropType = ctx.update.callback_query.data;
    ...
    syrup = ${syrups[ctx.wizard.state.coffeeSetup.syropType].name}
    `);
...
// const superWizardAdmin = new WizardScene('super-wizard',
//   async (ctx) => {
//     const a = superWizard;
//     await ctx.reply('Step 1. Chose your coffee', Markup.inlineKeyboard([
//       Markup.callbackButton(coffees.espresso.name, coffees.espresso.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.doppio.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.americano.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.cappuccino.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.latte.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.flat_white.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.raf.key),
//       Markup.callbackButton(coffees.espresso.name, coffees.cocoa.key),
//     ], { columns: 2 }).extra());
//     return ctx.wizard.next();
//   },
//   (ctx) => {
...
// const botAdmin = new Telegraf(tokenAdmin);
// const stageAdmin = new Stage([superWizardAdmin], { default: 'super-wizard' });
// botAdmin.use(session());
// botAdmin.use(stageAdmin.middleware());
// botAdmin.launch();
